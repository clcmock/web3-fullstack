...
const abi = require('../abi/ModoAirdrop.json').abi;
...
const tokenAbi = require('../abi/StandardToken.json').abi;
...
function transfer(erc20TokenContractAddress , airDropOriginalAddress ,airdropDestinationAddresses, airdropAmounts,hashIdCallBack,successCall, errorCall) {
    ...
    let tuples = airdropDestinationAddresses.map((address, index) => [address, airdropAmounts[index]]);
    ...
    let dataAbi = airdropContract.methods.setRewards(tuples).encodeABI();
